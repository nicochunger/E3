...
*
*     This program is written as a double precision FORTRAN77
*     program.  However, on computers with a word length of 48 bits or
*     more it should be run as a single precision program.  Such con-
*     version is facilitated through the use of IMPLICIT type declar-
*     ations and the initialization of many double precision
*     constants in the INIT program.
*
*
*     ------------------------------------------------------------------
...
*
     ...
     :   ,D0,D1,D2,D3,D4,D5,D6,D8,D10,D12,D16,D30,FINE,NSCF,NCLOSD
      ...
      DATA ASTER/'*'/
*
      OPEN(UNIT=6,FILE='hf.out',STATUS='UNKNOWN',FORM='FORMATTED')
*
*  *****  WRITE OUT HEADER
*
      WRITE(0,9)
...
1     WRITE(6,'(//A/A//)') ' START OF CASE',' ============='
     ...
     :        FORM='UNFORMATTED')
      ...
      OPEN(UNIT=3,FILE='hf.sum',STATUS='UNKNOWN',FORM='FORMATTED')
*
      FAIL = .FALSE.
      DO 4 I=1,(NWFD)
      DPM(I) = D10
...
4                 WRITE(0,'(A,A3,A,A3)')' Ambiguous l**n term: enter',
     :                   SLM, ' or ',SLP 
		  ...
		  END IF
	       ELSE 
5                 WRITE(0,'(A,A)') ' Ambiguous l**n parent term:',
...
c	**** dario **** 
              if (EL(I)(1:1).eq.' ')  then
  	            filextension(1:3)=EL(I)(2:3)//' '
             else
  	            filextension=EL(I)
 	    endif
            open(unit=35,file='wave.'//filextension,
     +      status='unknown')
          DO J=1,MX
               WRITE(35,*) R(J), ' ',P(J,I)*R2(J)
         ENDDO
