...
c
c     Dario
	integer :: iplane,jplane,kplane,imin,imax,jmin,jmax,kmin,kmax
	real(dp) :: dx,dy,dz
...
c.... Dario
c.... Writing in x,y,function format for 3d-plot
	open(50,file='density.dat',status='unknown')
	write(6,*) 'Plotting only one plane, defined by xmx/i ymx/j or '
     +            ,'zmx/k'
	write(6,*) ' For example: the plane z=zmax/2 is (0,0,2)'
	iplane=0
	jplane=0
	kplane=2
	write(6,*) ' Give i, j and k'
	read(5,*) iplane,jplane,kplane
...
c..... Dario
c
c      write(30,'(5g15.7)') (((plotbox(ii,jj,kk),ii=1,nw(1))
c     1     ,jj=1,nw(2)),kk=1,nw(3))
	dx = hh(1)
	dy = hh(2)
	dz = hh(3)
	imin = 1
	...
	do kk=kmin,kmax
	   do jj=jmin,jmax
		do ii=imin,imax
            write(50,*) ii*dx,' ',jj*dy,' ',kk*dz,' ',plotbox(ii,jj,kk)
		enddo
	   enddo
	enddo
c
